...
class Animation {
    ...
    // Method to turn the screen black
    turnScreenBlack() {
        for (let i = 0; i < this.numberOfFrames; i++) {
            for (let j = 0; j < this.pixels; j++) {
                this.animation[i][j] = color(0, 0, 0);
    ...
    // Method to check if the pixel is correct
    checkPixel() {
        let center = parseInt(this.pixels / 2);
        let k = this.currentFrameCount;
        if (this.animation[this.currentFrameCount][center + k] !== color(255, 255, 0) &&
            this.animation[this.currentFrameCount][center - k] !== color(255, 255, 0)) {
            this.turnScreenBlack();
        }
    }
}